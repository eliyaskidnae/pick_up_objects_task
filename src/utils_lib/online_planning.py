...
class StateValidityChecker:
    ...
    # Constructor
    def __init__(self, distance=0.05, is_unknown_valid=True , is_rrt_star = False):
    ...
    def is_valid(self, pose , exclude = []): 
        ...
        reserve_locations = [(-1.5, -1.5) , (1.5, 1.5) , (1.25, 0.5), (1.25, -1.25), (0, -1.25),(-0.5, 1.25) ,(-1.25, 0.5)]
        
        m = self.__position_to_map__(pose)
        grid_distance = int(self.distance/self.resolution)
        # add 6 points upper and to lower limit 
        # print("Pose in reserve list check" , pose in reserve_locations)
        if(exclude == [] and np.any([self.get_distance(loc,pose) < 0.35 for loc in reserve_locations])):
            return True
        elif( np.any([self.get_distance(loc,pose) < 0.3 for loc in exclude])):
        
            return True
        
        lower_x , lower_y = m[0] - grid_distance , m[1] - grid_distance  
        for lx in range(0,int(1.9*grid_distance)):
            for ly in range(0,int(1.9*grid_distance)):
...
def compute_path(start_p, goal_p, svc, bounds , max_time=1.0):
    ...
    rrt = RRT(svc ,10000 ,1.5, 0.2 , bounds, max_time , svc.is_rrt_star)
