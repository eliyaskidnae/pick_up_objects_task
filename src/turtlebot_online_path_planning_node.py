...
class OnlinePlanner:
    ...
    def get_goal(self, goal):
        ...
        if self.svc.there_is_map: 
            ...
            #     rospy.logwarn("Goal Point is not valid 1 , please try again")
    ...
    def plan(self):
        ...
        self.path = []
        if(not self.svc.is_valid(self.current_pose[0:2])):
            rospy.logwarn("Current is not valid, Target place is not safe")
            self.planning = False
...
if __name__ == '__main__':
    ...
    node = OnlinePlanner('/projected_map', '/odom', '/cmd_vel', np.array([-5.0, 5.0, -5.0, 5.0]), 
